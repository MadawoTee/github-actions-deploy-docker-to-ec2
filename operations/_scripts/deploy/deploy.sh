...
# Generate assessmentops config
/bin/bash $GITHUB_ACTION_PATH/operations/_scripts/generate/generate_assessmentops_config.sh
...
if [[ $SKIP_ASSESSMENTOPS_RUN == "true" ]]; then
  exit 1
fi

echo "::group::AssessmentOps Excecution"  
echo "Running AssessmentOps for env: $ASSESSMENTOPS_ENVIRONMENT"
docker run --rm --name assessmentops \
-e AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY_ID}" \
-e AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY}" \
-e AWS_SESSION_TOKEN="${AWS_SESSION_TOKEN}" \
-e AWS_DEFAULT_REGION="${AWS_DEFAULT_REGION}" \
-e ASSESSMENTOPS_ENVIRONMENT="${ASSESSMENTOPS_ENVIRONMENT}" \
-e SKIP_DEPLOY_TERRAFORM="${SKIP_DEPLOY_TERRAFORM}" \
-e SKIP_DEPLOY_HELM="${SKIP_DEPLOY_HELM}" \
-e ASSESSMENTOPS_TERRAFORM_COMMAND="${TERRAFORM_COMMAND}" \
